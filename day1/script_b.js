...
function turnRight(value) {
    let resultGreaterThanHundred = false;
    const reducedValue = value % 100;
    const result = currentValue + reducedValue;
    if (result > 100) {
        resultGreaterThanHundred = true;
        return [result % 100, resultGreaterThanHundred]
    } else {
        return [result % 100, resultGreaterThanHundred]
    }
}
...
function turnLeft(value) {
    ...
    if (result < 0) {
        ...
        result = result + 100
        return [result, resultLessThanZero]
    } else {
        return [result, resultLessThanZero]
    }
}
...
function solution() {
    ...
    dataArray.forEach((element) => {
        ...
        if (element.charAt(0) === "R") {
            const returnArray = turnRight(elementNumber)
            currentValue = returnArray[0];
            if (returnArray[1] === true) {
                numOfZeros++;
            }
        } else {
            const returnArray = turnLeft(elementNumber)
            currentValue = returnArray[0];
            if (returnArray[1] === true) {
                numOfZeros++;
            }
